...
export const getDashboardStats = async (req, res) => {
    try {
        ...
        const totalRevenue = totalRevenueBookings.reduce((sum, booking) => sum + booking.price, 0);

        // Get recent bookings (last 5)
        const recentBookings = await Booking.find({ owner: ownerId })
            .populate('car', 'brand model image category location year seating_capacity transmission')
