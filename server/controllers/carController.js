...
export const addCar = async (req, res) => {
    try {
        ...
        // Validation
        if (!brand || !model || !year || !pricePerDay || !category || !transmission || 
            !fuel_type || !seating_capacity || !location || !description) {
...
export const toggleCarAvailability = async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: true,
            message: `Car ${car.isAvailable ? 'activated' : 'deactivated'} successfully`,
            car
        });
    } catch (error) {
